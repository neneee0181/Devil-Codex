...
function isExternalModel(model: string | null | undefined): boolean {
  return Boolean(model && /^(copilot|claude-code|antigravity|openai|anthropic|google|deepseek|xai|openrouter|openrouter-free|opencode-free|groq|mistral|cerebras|together|fireworks|moonshot|huggingface|nvidia|ollama|vllm|lm-studio)(@[^/:]+)?[/:]/.test(model));
}
