...
export interface DevilCodexApi {
  ...
  browserRegister: (input: { key: string; webContentsId: number }) => Promise<BrowserState>;
  browserFocus: (input: { key: string }) => Promise<BrowserState>;
  browserNavigate: (input: { key?: string; url: string }) => Promise<void>;
  browserBack: (input?: { key?: string }) => Promise<void>;
  browserForward: (input?: { key?: string }) => Promise<void>;
  browserReload: (input?: { key?: string }) => Promise<void>;
  browserStop: (input?: { key?: string }) => Promise<void>;
  browserHardReload: (input?: { key?: string }) => Promise<void>;
  browserState: (input?: { key?: string }) => Promise<BrowserState>;
  browserScreenshot: (input?: { key?: string }) => Promise<string>;
  browserFind: (input: { key?: string; text: string; forward?: boolean; findNext?: boolean }) => Promise<void>;
  browserStopFind: (input?: { key?: string }) => Promise<void>;
  browserZoom: (input: { key?: string; factor?: number; delta?: number; reset?: boolean }) => Promise<number>;
  browserClearCookies: (input?: { key?: string }) => Promise<void>;
  browserClearCache: (input?: { key?: string }) => Promise<void>;
  browserCaptureRect: (input: { key?: string; x: number; y: number; width: number; height: number }) => Promise<string>;
  browserAiClick: (input: { key?: string; x?: number; y?: number; selector?: string }) => Promise<boolean>;
  browserAiType: (input: { key?: string; text: string }) => Promise<void>;
  browserUploadFiles: (input: { key?: string; paths: string[] }) => Promise<{ ok: boolean; count: number; detail?: string }>;
  browserAiKey: (input: { key?: string }) => Promise<void>;
  browserAiScroll: (input: { key?: string; dy: number }) => Promise<void>;
  browserAiRead: () => Promise<string>;
  onBrowserState: (listener: (event: { key: string; state: BrowserState }) => void) => () => void;
  onBrowserActivate: (listener: () => void) => () => void;
  onBrowserNewTab: (listener: (event: { sourceKey: string; url: string }) => void) => () => void;
